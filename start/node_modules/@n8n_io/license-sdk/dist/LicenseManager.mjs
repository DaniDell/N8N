import f from"crypto-js";import{machineId as A}from"node-machine-id";import D from"node-rsa";import{X509Certificate as p}from"crypto";import{fetch as C,getGlobalDispatcher as w,setGlobalDispatcher as v,Dispatcher as y,ProxyAgent as I}from"undici";import{URL as E}from"url";function c(a){let e=process.env[a];return e?new I(e):null}var g=c("http_proxy_license_server")||c("http_proxy"),d=c("https_proxy_license_server")||c("https_proxy"),T={...g?{"http:":g}:{},...d?{"https:":d}:{}},b=(process.env.no_proxy??"").split(",").map(a=>a.trim()),F=w();v(new class extends y{dispatch(a,e){if(a.origin){let{host:t,protocol:n}=typeof a.origin=="string"?new E(a.origin):a.origin;if(!b.some(i=>i.startsWith(".")?t.endsWith(i):t===i)){let i=T[n];i&&i.dispatch(a,e)}}return F.dispatch(a,e)}});async function u(a,e,t={}){let n=new AbortController,i=setTimeout(()=>{n.abort()},t.timeoutInMs??3e4);try{let r=await C(a,{method:"post",body:JSON.stringify(e),headers:{"Content-Type":"application/json"},signal:n.signal}),s=await r.json();return{status:r.status,data:s}}catch(r){throw new Error("Connection Error",{cause:r})}finally{clearTimeout(i)}}var m=class{constructor(e){this.isInitializationCompleted=!1;this.isShuttingDown=!1;this.config=e,this.config.logger?this.logger=this.config.logger:this.logger={error(){console.log("ERROR:",...arguments)},warn(){console.log("WARN:",...arguments)},info(){console.log("INFO:",...arguments)},debug(){console.log("DEBUG:",...arguments)}},this.x509IssuerCert=new p(`-----BEGIN CERTIFICATE-----
