"use strict";var b=Object.create;var l=Object.defineProperty;var F=Object.getOwnPropertyDescriptor;var A=Object.getOwnPropertyNames;var D=Object.getPrototypeOf,R=Object.prototype.hasOwnProperty;var x=(s,e)=>{for(var t in e)l(s,t,{get:e[t],enumerable:!0})},C=(s,e,t,n)=>{if(e&&typeof e=="object"||typeof e=="function")for(let i of A(e))!R.call(s,i)&&i!==t&&l(s,i,{get:()=>e[i],enumerable:!(n=F(e,i))||n.enumerable});return s};var w=(s,e,t)=>(t=s!=null?b(D(s)):{},C(e||!s||!s.__esModule?l(t,"default",{value:s,enumerable:!0}):t,s)),L=s=>C(l({},"__esModule",{value:!0}),s);var O={};x(O,{LicenseManager:()=>p});module.exports=L(O);var d=w(require("crypto-js")),E=require("node-machine-id"),T=w(require("node-rsa")),f=require("crypto");var o=require("undici"),I=require("url");function u(s){let e=process.env[s];return e?new o.ProxyAgent(e):null}var v=u("http_proxy_license_server")||u("http_proxy"),y=u("https_proxy_license_server")||u("https_proxy"),M={...v?{"http:":v}:{},...y?{"https:":y}:{}},S=(process.env.no_proxy??"").split(",").map(s=>s.trim()),N=(0,o.getGlobalDispatcher)();(0,o.setGlobalDispatcher)(new class extends o.Dispatcher{dispatch(s,e){if(s.origin){let{host:t,protocol:n}=typeof s.origin=="string"?new I.URL(s.origin):s.origin;if(!S.some(i=>i.startsWith(".")?t.endsWith(i):t===i)){let i=M[n];i&&i.dispatch(s,e)}}return N.dispatch(s,e)}});async function g(s,e,t={}){let n=new AbortController,i=setTimeout(()=>{n.abort()},t.timeoutInMs??3e4);try{let r=await(0,o.fetch)(s,{method:"post",body:JSON.stringify(e),headers:{"Content-Type":"application/json"},signal:n.signal}),a=await r.json();return{status:r.status,data:a}}catch(r){throw new Error("Connection Error",{cause:r})}finally{clearTimeout(i)}}var p=class{constructor(e){this.isInitializationCompleted=!1;this.isShuttingDown=!1;this.config=e,this.config.logger?this.logger=this.config.logger:this.logger={error(){console.log("ERROR:",...arguments)},warn(){console.log("WARN:",...arguments)},info(){console.log("INFO:",...arguments)},debug(){console.log("DEBUG:",...arguments)}},this.x509IssuerCert=new f.X509Certificate(`-----BEGIN CERTIFICATE-----
...
`).join("")}triggerOnFeatureChangeCallback(){this.config.onFeatureChange&&this.config.onFeatureChange(this.currentFeatures??{})}setTimerForNextEntitlementChange(){if(!this.hasCert())return;let e=new Date().getTime(),t=new Set;t.add(this.licenseCert.expiresAt.getTime()-e),t.add(this.licenseCert.terminatesAt.getTime()-e);let n=[...this.licenseCert.entitlements.reduce((r,a)=>(r.add(a.validFrom.getTime()-e),r.add(a.validTo.getTime()-e),r),t)].filter(r=>r>=0&&r<=15*60*1e3).sort((r,a)=>r-a);if(n.length===0)return;let i=n[0];clearTimeout(this.entitlementChangeTimeoutPointer),this.entitlementChangeTimeoutPointer=setTimeout(()=>{this.updateCurrentFeatures(),this.triggerOnFeatureChangeCallback(),this.setTimerForNextEntitlementChange()},i+1e3)}async renewalCron({force:e}){if(e||this.isRenewalDue()){this.log("attempting license renewal","debug");try{await this._renew({cause:e?"startup":"auto"})}catch{}}}async initCert(){if(!this.isShuttingDown)try{let e=await this.config.loadCertStr();if(!e){this.licenseCert=void 0;return}if(e.length<50)throw new Error("cert string is undefined or too short");let{x509:t,licenseKey:n}=this.parseLicenseCertContainerStr(e);if(this.x509Cert=new f.X509Certificate(t),!this.x509Cert.checkIssued(this.getIssuerCert()))throw new Error("cert was not issued by an approved issuer");let r=this.x509Cert.publicKey.export({format:"pem",type:"pkcs1"});this.key=new T.default(r),this.licenseCert=this.parseLicenseKeyStr(n),this.updateCurrentFeatures(),this.triggerOnFeatureChangeCallback(),this.setTimerForNextEntitlementChange()}catch(e){this.log(`cert could not be initialized. ${e instanceof Error?e.message:""}`,"error")}}stringifyCertContainer(e){let t=JSON.stringify(e);return Buffer.from(t).toString("base64")}parseLicenseCertContainerStr(e){let t=Buffer.from(e,"base64").toString("ascii"),n=JSON.parse(t);if(!n.hasOwnProperty("licenseKey")||!n.hasOwnProperty("x509"))throw new Error("license cert container could not be parsed");return n}parseLicenseKeyStr(e){let t=this.validateLicenseKey(e),n={...t};return n.createdAt=new Date(t.createdAt),n.issuedAt=new Date(t.issuedAt),n.expiresAt=new Date(t.expiresAt),n.terminatesAt=new Date(t.terminatesAt),n.entitlements=n.entitlements.map(i=>(i.validFrom=new Date(i.validFrom),i.validTo=new Date(i.validTo),i)),n}validateLicenseKey(e){e=e.replace(/(\r\n|\n|\r)/gm,"");let t=e.match(/^-----BEGIN LICENSE KEY-----(?<encryptedSymmetricKey>.+\|\|)(?<encryptedData>.+)\|\|(?<signature>.+)-----END LICENSE KEY-----$/);if(!t)throw new Error("license key could not be parsed");let n=t.groups.encryptedSymmetricKey,i=t.groups.encryptedData,r=t.groups.signature,a,c;try{a=this.key.decryptPublic(n,"utf8")}catch{throw new Error("Invalid data: Could not extract symmetric key")}try{c=d.default.AES.decrypt(i,a).toString(d.default.enc.Utf8)}catch{throw new Error("Invalid Data: Could not decrypt data with key found")}if(this.key.verify(Buffer.from(c),r,"utf8","base64"))return JSON.parse(c);throw new Error("License Key signature invalid")}getIssuerCert(){return this.x509IssuerCert}async shutdown(){if(this.isShuttingDown=!0,this.config.autoRenewEnabled&&clearInterval(this.renewalIntervalPointer),this.checkUpcomingEntitlementChangesCron&&clearInterval(this.checkUpcomingEntitlementChangesCron),this.entitlementChangeTimeoutPointer&&clearTimeout(this.entitlementChangeTimeoutPointer),!this.hasCert())return;this.licenseCert.entitlements.some(t=>t.isFloatable??!1)&&await this._renew({detachFloatingEntitlements:!0,cause:"shutdown"})}};0&&(module.exports={LicenseManager});
