...
const executionDataRecovery_service_1 = require("../executionDataRecovery.service");
...
let MessageEventBus = class MessageEventBus extends events_1.default {
    constructor(logger, executionRepository, eventDestinationsRepository, workflowRepository, orchestrationService, recoveryService) {
        super();
        this.logger = logger;
        ...
        if (options === null || options === void 0 ? void 0 : options.skipRecoveryPass) {
            ...
            if (config_1.default.get('executions.mode') !== 'queue') {
                const dbUnfinishedExecutionIds = (await this.executionRepository.find({
                    where: {
                        status: (0, typeorm_1.In)(['running', 'new', 'unknown']),
                    },
                    select: ['id'],
                })).map((e) => e.id);
            ...
            if (unfinishedExecutionIds.length > 0) {
                ...
                if (recoveryAlreadyAttempted || config_1.default.getEnv('eventBus.crashRecoveryMode') === 'simple') {
                    ...
                    for (const executionId of unfinishedExecutionIds) {
                        ...
                        if (!((_f = unsentAndUnfinished.unfinishedExecutions[executionId]) === null || _f === void 0 ? void 0 : _f.length)) {
                            this.logger.debug(`No event messages found, marking execution ${executionId} as 'crashed'`);
                            await this.executionRepository.markAsCrashed([executionId]);
                        }
                        else {
                            await this.recoveryService.recoverExecutionData(executionId, unsentAndUnfinished.unfinishedExecutions[executionId], true);
                        }
                    }
                }
                (_g = this.logWriter) === null || _g === void 0 ? void 0 : _g.endRecoveryProcess();
            }
        }
    ...
    async handleRedisEventBusMessage(messageString) {
        const eventData = (0, n8n_workflow_1.jsonParse)(messageString);
        if (eventData) {
...
exports.MessageEventBus = MessageEventBus = __decorate([
    ...
    __metadata("design:paramtypes", [Logger_1.Logger,
        ...
        executionDataRecovery_service_1.ExecutionDataRecoveryService])
], MessageEventBus);
