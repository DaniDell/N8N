...
let WorkflowExecutionService = class WorkflowExecutionService {
    constructor(logger, executionRepository, workflowRepository, nodeTypes, testWebhooks, permissionChecker, workflowRunner) {
        this.logger = logger;
    ...
    async executeManually({ workflowData, runData, pinData, startNodes, destinationNode, }, user, pushRef) {
        const pinnedTrigger = this.selectPinnedActivatorStarter(workflowData, startNodes === null || startNodes === void 0 ? void 0 : startNodes.map((nodeData) => nodeData.name), pinData);
    ...
    async executeErrorWorkflow(workflowId, workflowErrorData, runningProject) {
        ...
        try {
            ...
            try {
                const failedNode = ((_a = workflowErrorData.execution) === null || _a === void 0 ? void 0 : _a.lastNodeExecuted)
                    ? workflowInstance.getNode((_b = workflowErrorData.execution) === null || _b === void 0 ? void 0 : _b.lastNodeExecuted)
                    : undefined;
                await this.permissionChecker.checkSubworkflowExecutePolicy(workflowInstance, workflowErrorData.workflow.id, failedNode !== null && failedNode !== void 0 ? failedNode : undefined);
            }
            catch (error) {
            ...
            if (workflowStartNode === undefined) {
                this.logger.error(`Calling Error Workflow for "${workflowErrorData.workflow.id}". Could not find "${ERROR_TRIGGER_TYPE}" in workflow "${workflowId}"`);
                return;
            }
            const nodeExecutionStack = [];
    ...
    selectPinnedActivatorStarter(workflow, startNodes, pinData) {
        ...
        if ((startNodes === null || startNodes === void 0 ? void 0 : startNodes.length) === 0)
            return firstPinnedActivator !== null && firstPinnedActivator !== void 0 ? firstPinnedActivator : null;
        ...
        if (parentNodeNames.length > 0) {
            ...
            return (_a = allPinnedActivators.find((pa) => pa.name === parentNodeName)) !== null && _a !== void 0 ? _a : null;
        }
        return (_b = allPinnedActivators.find((pa) => pa.name === firstStartNodeName)) !== null && _b !== void 0 ? _b : null;
    }
    findAllPinnedActivators(workflow, pinData) {
        return workflow.nodes
            .filter((node) => !node.disabled &&
            (pinData === null || pinData === void 0 ? void 0 : pinData[node.name]) &&
...
exports.WorkflowExecutionService = WorkflowExecutionService = __decorate([
    ...
    __metadata("design:paramtypes", [Logger_1.Logger,
        ...
        TestWebhooks_1.TestWebhooks,
        PermissionChecker_1.PermissionChecker,
        WorkflowRunner_1.WorkflowRunner])
], WorkflowExecutionService);
//# sourceMappingURL=workflowExecution.service.js.map