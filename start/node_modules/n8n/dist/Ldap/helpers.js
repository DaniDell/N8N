...
const getAuthIdentityByLdapId = async (idAttributeValue) => {
    return await typedi_1.Container.get(authIdentity_repository_1.AuthIdentityRepository).findOne({
        relations: { user: true },
        where: {
...
const getLdapIds = async () => {
    const identities = await typedi_1.Container.get(authIdentity_repository_1.AuthIdentityRepository).find({
        select: ['providerId'],
        where: {
            providerType: 'ldap',
        },
    ...
    await Db.transaction(async (transactionManager) => {
        return await Promise.all([
            ...toCreateUsers.map(async ([ldapId, user]) => {
                ...
                const authIdentity = AuthIdentity_1.AuthIdentity.create(savedUser, ldapId);
...
const saveLdapSynchronization = async (data) => {
    await typedi_1.Container.get(authProviderSyncHistory_repository_1.AuthProviderSyncHistoryRepository).save({
        ...data,
        providerType: 'ldap',
    }, { transaction: false });
};
...
const getLdapSynchronizations = async (page, perPage) => {
    ...
    return await typedi_1.Container.get(authProviderSyncHistory_repository_1.AuthProviderSyncHistoryRepository).find({
...
const createLdapAuthIdentity = async (user, ldapId) => {
    return await typedi_1.Container.get(authIdentity_repository_1.AuthIdentityRepository).save(AuthIdentity_1.AuthIdentity.create(user, ldapId), {
        transaction: false,
    });
};
...
const deleteAllLdapIdentities = async () => {
    return await typedi_1.Container.get(authIdentity_repository_1.AuthIdentityRepository).delete({ providerType: 'ldap' });
};
