...
function getRedisStandardClient(redis, redisOptions, redisType) {
    ...
    if (config_1.default.getEnv('queue.bull.redis.tls'))
        sharedRedisOptions.tls = {};
    const logger = typedi_1.default.get(Logger_1.Logger);
    logger.debug(`Initialising Redis client${redisType ? ` of type ${redisType}` : ''} connection with host: ${host !== null && host !== void 0 ? host : 'localhost'} and port: ${port !== null && port !== void 0 ? port : '6379'}`);
    return new redis({
        ...sharedRedisOptions,
        retryStrategy: () => {
            const now = Date.now();
...
function getRedisClusterClient(redis, redisOptions, redisType) {
    ...
    if (config_1.default.getEnv('queue.bull.redis.tls'))
        sharedRedisOptions.tls = {};
    const logger = typedi_1.default.get(Logger_1.Logger);
    logger.debug(`Initialising Redis cluster${redisType ? ` of type ${redisType}` : ''} connection with nodes: ${clusterNodes.map((e) => `${e.host}:${e.port}`).join(',')}`);
